...
class AppFixtures extends Fixture
{
    ...
    public function load(ObjectManager $manager): void
    {
        ...
        for ($i = 0; $i < 10; $i++) {
            ...
            $pdfFilePath = 'public/uploads/pdfs/tableau-caractéristiques-'. $i .'.pdf';
    ...
    {
        $relativePath = [
            'public/images/Voiture1.png',
            'public/images/Voiture2.png',
            'public/images/Voiture3.png',
        ];
    
        $randomIndex = array_rand($relativePath);
        $randomImagePath = $relativePath[$randomIndex];
    
        // Ajouter un message de débogage pour vérifier le chemin
        echo "Chemin du fichier : $randomImagePath\n";
    
        // Obtenir le nom de fichier à partir du chemin
        $fileName = pathinfo($randomImagePath, PATHINFO_BASENAME);
    
        // Chemin du répertoire temporaire
        $tmpDir = sys_get_temp_dir() . '/uploaded_files_' . uniqid();
        ...
        copy($randomImagePath, $tmpPath);
    
        // Retourner l'instance UploadedFile avec le chemin temporaire
        return new UploadedFile($tmpPath, $fileName, mime_content_type($tmpPath), null, true);
    }
}
